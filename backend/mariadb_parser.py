#!/usr/bin/env python3
"""
Enhanced MariaDB Parser for Kidnapping Data Migration
Handles SQL INSERT statement parsing with proper error handling and data validation
"""
...
class MariaDBParser:
    """Enhanced parser for MariaDB SQL export files"""
    ...
    def parse_sql_export(self) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Starting to parse SQL export file: {self.sql_file_path}")
        ...
        if not all_inserts:
            logger.error("No conflicts INSERT statements found in SQL file")
            return []
        
        logger.info(f"Found {len(all_inserts)} INSERT statements")
        
        # Parse all INSERT statements
        all_records = []
        for i, values_str in enumerate(all_inserts):
            logger.info(f"Processing INSERT {i+1}/{len(all_inserts)}...")
            
            # Parse individual records from this INSERT statement
            records = self._parse_insert_values(values_str)
            all_records.extend(records)
            
            logger.info(f"Parsed {len(records)} records from INSERT {i+1}")
        
        logger.info(f"Successfully parsed {len(all_records)} total conflict records")
        
        self.records = all_records
        return all_records
    
    def _parse_insert_values(self, values_str: str) -> List[Dict[str, Any]]:
        ...
        for i, char in enumerate(values_str):
            if char in ("'", '"') and not in_quotes:
                ...
                if paren_count == 0:
                    record_count += 1
                    try:
                        parsed_record = self._parse_single_record(current_record.strip(), record_count)
                        if parsed_record:
        ...
        logger.info(f"Parsed {len(records)} valid records from {record_count} total records")
        return records
    
    def _parse_single_record(self, record_str: str, record_id: int) -> Optional[Dict[str, Any]]:
        ...
        if len(values) < 24:
            logger.warning(f"Record {record_id} has insufficient columns: {len(values)}")
            return None
        ...
        try:
            ...
            logger.error(f"Error parsing record {record_id}: {e}")
            return None
    ...
    def extract_kidnapping_records(self) -> List[Dict[str, Any]]:
        ...
        if not self.records:
            logger.warning("No records parsed yet, call parse_sql_export first")
            return []
        ...
        logger.info(f"Found {len(kidnapping_records)} records with kidnapping data")
