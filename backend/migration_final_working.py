#!/usr/bin/env python3
"""
Final working migration with NULL actors to avoid FK constraints
"""
...
def generate_working_migration():
    """Generate SQL file that will actually work"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    print(f"=== GENERATING WORKING MIGRATION ===")
    ...
    try:
        ...
        # Generate SQL file
        output_file = f"working_migration_{timestamp}.sql"
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"-- WORKING Kidnapping Data Migration\n")
            ...
            f.write(f"-- Insert all records with NULL actors\n")
            ...
            for i, record in enumerate(records):
                ...
                # 18-20. kidnapping counts (KEY DATA!)
                ...
                # 24-26. actors - use NULL to avoid FK issues
                values.append('NULL')
                values.append('NULL')
                values.append('NULL')
...
if __name__ == "__main__":
    sql_file = generate_working_migration()
    
    if sql_file:
        print(f"\n🎉 Ready to execute WORKING migration!")
