...
def execute_migration():
    ...
    print(f"Target: Replace 13 records with 1,107 kidnapping records (8,727 victims)")
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(text("SELECT COUNT(*) FROM public.conflicts"))
            old_count = result.scalar()
            print(f"   Current records: {old_count}")
            
            # Clear the table
            conn.execute(text("TRUNCATE TABLE public.conflicts"))
            conn.commit()
            ...
            for i, record in enumerate(records):
                try:
                    ...
                    if len(current_batch) >= batch_size:
                        ...
                        columns = list(current_batch[0].keys())
                        placeholders = ', '.join([f':{col}' for col in columns])
                        
                        sql = text(f"""
                            INSERT INTO public.conflicts ({', '.join(columns)})
                            VALUES ({placeholders})
                        """)
                        ...
                        kidnapping_in_batch = sum(1 for r in current_batch if r['kidnapped_male'] > 0 or r['kidnapped_female'] > 0 or r['kidnapped_unknown'] > 0)
                        kidnapping_inserted += kidnapping_in_batch
                        
                        print(f"   Progress: {inserted_count}/{len(records)} records inserted ({kidnapping_inserted} kidnapping)")
...
if __name__ == "__main__":
    ...
    if response.lower() == 'yes':
        ...
        if success:
            ...
            print(f"📈 Improvement: 13 → 1,107 kidnapping records (85x increase)")
            print(f"👥 Improvement: 78 → 8,727 victims (112x increase)")
        else:
