#!/usr/bin/env python3
"""
Simple migration with valid foreign keys
"""
...
def generate_simple_migration():
    """Generate SQL file with valid foreign keys"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    print(f"=== GENERATING SIMPLE MIGRATION ===")
    ...
    try:
        ...
        # Generate SQL file
        output_file = f"simple_migration_{timestamp}.sql"
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"-- SIMPLE Kidnapping Data Migration\n")
            ...
            f.write(f"-- Insert all records with valid foreign keys\n")
            ...
            for i, record in enumerate(records):
                # Format values with valid foreign keys
                values = []
                ...
                # 2-4. Use valid foreign keys (1 for country, 1-6 for region)
                values.append(str(record['conflict_type_id'] if record['conflict_type_id'] else 1))  # conflict_type_id
                values.append('1')  # country_id (Nigeria)
                
                # Map state to region (simplified mapping)
                state_id = record.get('state_id', 0)
                if state_id in [1, 2, 3, 4, 5, 6]:  # North East states
                    region_id = 1
                elif state_id in [7, 8, 9, 10, 11, 12]:  # North Central
                    region_id = 2
                elif state_id in [13, 14, 15, 16, 17, 18]:  # North West
                    region_id = 3
                elif state_id in [19, 20, 21, 22, 23, 24]:  # South East
                    region_id = 4
                elif state_id in [25, 26, 27, 28, 29, 30]:  # South South
                    region_id = 5
                else:  # South West or default
                    region_id = 6
                values.append(str(region_id))
                
                # 6-7. state_id, lga_id (use 0 if invalid)
                values.append(str(state_id) if state_id and state_id > 0 else '1')
                values.append(str(record['lga_id']) if record['lga_id'] and record['lga_id'] > 0 else '1')
                ...
                values.append('0')  # displaced_male
                values.append('0')  # displaced_female
                
                # 24-26. actors (use NULL for invalid)
                actor1_val = record.get('actor_1', 0)
                values.append(str(actor1_val) if actor1_val and actor1_val > 0 else 'NULL')
                values.append('NULL')  # actor_2
                values.append('NULL')  # actor_3
...
if __name__ == "__main__":
    sql_file = generate_simple_migration()
    
    if sql_file:
        print(f"\n🎉 Ready to execute SIMPLE migration!")
