#!/usr/bin/env python3
"""
Generate SQL INSERT statements for the complete migration
"""
...
def generate_migration_sql():
    ...
    print(f"=== GENERATING MIGRATION SQL ===")
    ...
    try:
        ...
        # Generate SQL file
        output_file = f"complete_migration_{timestamp}.sql"
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"-- Complete Kidnapping Data Migration\n")
            ...
            for i, record in enumerate(records):
                ...
                for field in ['community', 'displaced_persons', 'description', 'source_url', 'data_source']:
                    text_val = str(record[field]).replace("'", "''")
                    if text_val:
                        values.append(f"'{text_val}'")
                    else:
                        values.append("''")
